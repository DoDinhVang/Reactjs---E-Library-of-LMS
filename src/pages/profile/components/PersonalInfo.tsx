...
export default function PersonalInfo() {
        const {disabled} = useSelector((state:State) => state.profileReducer)
        const dispatch = useDispatch()
    return (
        <>
            <h3 className='title px-5 py-3 mb-0'>Thông tin chung</h3>
                    ...
                    <div className='row' style={{ flexBasis: '80%' }}>
                        <div className='col-6'>
                            <div className='userId d-flex mb-3'>
                                ...
                                <Input disabled = {disabled} size='large' defaultValue={'123'} style={{ width: '70%' }} />
                            </div>
                            <div className='userId d-flex  mb-3'>
                                <p className='mb-0' style={{ whiteSpace: 'nowrap', width: '30%', marginRight: '12px' }}>Giới tính</p>
                                <Select disabled = {disabled} defaultValue="lucy" style={{ width: 120, textAlign: 'left' }} onChange={() => { }}>
                            ...
                            <div className='userId d-flex mb-3'>
                                ...
                                <Input disabled =  {disabled} defaultValue={'Quản lý'} style={{ width: '70%' }} />
                            </div>
                            <div className='userId d-flex  mb-3'>
                                <p className='mb-0 me-3' style={{ whiteSpace: 'nowrap', width: '30%' }}>Email</p>
                                <Input disabled =  {disabled} style={{ width: '70%' }} defaultValue={'huu0302@gmail.com'} />
                            </div>
                        </div>
                        <div className='col-6'>
                            <div className='userId d-flex  mb-3'>
                                <p className='mb-0 me-3' style={{ whiteSpace: 'nowrap', width: '30%' }}>Tên người dùng</p>
                                <Input disabled =  {disabled} style={{ width: '70%' }} defaultValue={'admin123'} />
                            </div>
                            <div className='userId d-flex  mb-3'>
                                <p className='mb-0 me-3' style={{ whiteSpace: 'nowrap', width: '30%' }}>Số điện thoại</p>
                                <Input disabled =  {disabled} style={{ width: '70%' }} defaultValue={'12356789'} />
                            </div>
                            <div className='userId d-flex  mb-3'>
                                <p className='mb-0 me-3' style={{ whiteSpace: 'nowrap', width: '30%' }}>Địa chỉ</p>
                                <TextArea disabled =  {disabled} style={{ width: '70%' }} rows={4} defaultValue='hi tớ là chomeo' />
                            </div>
                        </div>
                    </div>
                </div>
                <div className='text-center'>
                    <button onClick={()=>{dispatch(setDisibleAttributeActionCreator(true))}} className='button-gray me-3'>Hủy</button>
